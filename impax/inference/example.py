...
def _get_world_pts_from_idx(
    ...
    while point_count < target_point_count:
        ...
        point_count = world_xyzn.shape[0]
    return world_xyzn[:target_point_count, :].astype(jnp.float32)
...
class InferenceExample(object):
    ...
    @property
    def data_dir(self):
        return "./impax/datadir"

    @property
    def npz_path(self):
        return f"{self.data_dir}/%s/%s/%s.npz" % (
            self.split,
            self.synset,
            self.mesh_hash,
        )
    ...
    def surface_samples_path(self):
        sp = self.npz_path.replace(f"{self.data_dir}", f"{self.data_dir}/surface-points/").replace(
    ...
    def dodeca_depth_and_normal_path(self):
        if not hasattr(self, "_dodeca_depth_and_normal_path"):
            self._dodeca_depth_and_normal_path = (
                self.data_dir + f"/{self.split}/{self.synset}/{self.mesh_hash[:-1]}.npz"
            )
    ...
    def r2n2_depth_dir(self):
        ...
        root = f"{self.data_dir}/3dr2n2-renders/3dr2n2/"
    ...
    def v12occnet_path(self):
        root = f"{self.data_dir}/v1shapenet2occnet/"
    ...
    def r2n2_normal_cam_images(self):
        ...
        if not hasattr(self, "_r2n2_normal_cam_images"):
            ...
            for i in range(24):
                ...
                cam_images.append(jnp_util.zero_by_mask(mask, im_i).astype(jnp.float32))
            self._r2n2_normal_cam_images = jnp.stack(cam_images)
    ...
    def r2n2_xyz_images(self):
        ...
        if not hasattr(self, "_r2n2_xyz_images"):
            ...
            self._r2n2_xyz_images = jnp.stack(xyz_images)
        return self._r2n2_xyz_images

    def set_r2n2_cam2v1(self, cam2v1):
        assert not hasattr(self, "_r2n2_cam2v1")
        self._r2n2_cam2v1 = jnp.reshape(cam2v1, [24, 4, 4])
        self._r2n2_xfov = 0.422204 + jnp.zeros([24], dtype=jnp.float32)
    ...
    def r2n2_viewpoints(self):
        ...
        if not hasattr(self, "_r2n2_viewpoints"):
            ...
            metadata_path = (
                f"{self.data_dir}/3dr2n2-renders/" f"{self.synset}/{self.mesh_hash}/rendering/rendering_metadata.txt"
            )
    ...
    def occnet2v1(self):
        if not hasattr(self, "_occnet2v1"):
            self._occnet2v1 = assert_is_4x4(jnp.linalg.inv(self.v12occnet))
        return self._occnet2v1

    @property
    def gaps2occnet(self):
        if not hasattr(self, "_gaps2occnet"):
            self._gaps2occnet = jnp.linalg.inv(self.occnet2gaps)
    ...
    def world_normal_images(self):
        ...
        if not hasattr(self, "_world_normal_images"):
            ...
            for i in range(20):
                ...
                cam2world_invt = jnp.linalg.inv(cam2world[i, ...]).T
                im_i = geom_util_jnp.apply_4x4(im_i, cam2world_invt, are_points=False)
                nrm = jnp.linalg.norm(im_i, axis=-1, keepdims=True) + 1e-10
                im_i /= nrm
    ...
    def cam_xyz_images_from_dodeca(self):
        if not hasattr(self, "_cam_xyz_images_from_dodeca"):
            ...
            self._cam_xyz_images_from_dodeca = jnp.stack(cam_images)
    ...
    def get_surface_samples_from_single_dodeca_image(self, idx):
        ...
        is_valid = jnp.reshape(is_valid, [224, 224])
    ...
    def precomputed_surface_samples_from_dodeca(self):
        if not hasattr(self, "_precomputed_surface_samples_from_dodeca"):
            ...
            while full_samples.shape[0] < self.surface_sample_count:
                log.info(f"Doubling samples from {full_samples.shape[0]} to" f" {2*full_samples.shape[0]}")
    ...
    def surface_samples_from_dodeca(self):
        ...
        if not hasattr(self, "_surface_samples_from_dodeca"):
            ...
            is_valid = jnp.reshape(is_valid, [20, 224, 224])
            ...
            while world_xyzn.shape[0] < self.surface_sample_count:
                log.info(f"Tiling samples from {world_xyzn.shape[0]} to" f" {2*world_xyzn.shape[0]}")
                world_xyzn = jnp.tile(world_xyzn, [2, 1])
    ...
    def gt_path(self):
        if self._gt_path is None:
            self._gt_path = (self.data_dir + "/%s/%s/%s/mesh_orig.ply") % (
                self.split,
                self.synset,
                self.mesh_hash,
            )
    ...
    def tx_path(self):
        if not self._tx_path:
            self._tx_path = (self.data_dir + "/%s/%s/%s/orig_to_gaps.txt") % (
                self.split,
                self.synset,
                self.mesh_hash,
            )
    ...
    def r2n2_cam2world(self):
        ...
        if not hasattr(self, "_r2n2_cam2world"):
            ...
            for i in range(24):
                ...
                cam2occnet = jnp.matmul(v12occnet, cam2v1)
                cam2gaps = jnp.matmul(occnet2gaps, cam2occnet)
                ms.append(assert_is_4x4(cam2gaps))
            self._r2n2_cam2world = jnp.stack(ms).astype(jnp.float32)
    ...
    def tx(self):
        if self._tx is None:
            ...
            self._tx = jnp.reshape(tx, [4, 4])
        return self._tx

    @property
    def gaps_to_occnet(self):
        if self._gaps_to_occnet is None:
            self._gaps_to_occnet = jnp.linalg.inv(self.tx)
    ...
    def max_depth_normals_path(self):
        return ("{self.data_dir}/max/" "depth-normals-npz/%s/%s/%s.npz") % (
            self.split,
            self.synset,
            self.mesh_hash,
        )
    ...
    def max_cam_xyz(self):
        if not hasattr(self, "_max_cam_xyz"):
            ...
            self._max_cam_xyz = geom_util_jnp.depth_images_to_cam_images(depths)
    ...
    def max_cam2occnet_path(self):
        return ("{self.data_dir}/occnet_scan_64_pkls_noclip/%s/%s/%s/occnet_scan.pkl") % (
            self.split,
            self.synset,
            self.mesh_hash,
        )
    ...
    def max_cam2world(self):
        ...
        if not hasattr(self, "_max_cam2world"):
            ...
            cam2worlds = []
            for i in range(16):
                cam2worlds.append(jnp.matmul(occnet2gaps, cam2occnet[i, :, :]))
            self._max_cam2world = jnp.stack(cam2worlds)
    ...
    def max_world_xyz(self):
        if not hasattr(self, "_max_world_xyz"):
            ...
            world = geom_util_jnp.batch_apply_4x4(cam, self.max_cam2world)
            is_invalid = jnp.squeeze(self.max_depth_512 == 0.0)
    ...
    def bts_world_xyz(self):
        if not hasattr(self, "_bts_world_xyz"):
            ...
            world = geom_util_jnp.batch_apply_4x4(cam, self.bts_cam2world)
            is_invalid = jnp.squeeze(self.bts_depth_480 == 0.0)
            world[is_invalid, :] = 0.0
    ...
    def _batch_resize(self, ims, res, strategy="nearest"):
        ...
        out = jnp.stack(out)
        if not has_extra_dim:
            out = jnp.reshape(out, out.shape[:-1])
        return out
    ...
    def bts_depth_480(self):
        ...
        if not hasattr(self, "_bts_depth_480"):
            ...
            for i in range(24):
                use_gt = False
                if use_gt:
                    path = ("{self.data_dir}/bts-pred/depth-gt/" "%s/%s/%s_depth.png") % (
                        self.synset,
                        self.mesh_hash,
                        str(i).zfill(6),
                    )
