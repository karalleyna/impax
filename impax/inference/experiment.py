...
class Job(object):
    ...
    def __init__(self, parent_experiment, job_str, use_temp_ckpts="warn"):
        ...
        self.lazy_attrs = ["_all_checkpoints", "_root_dir", "_hparams", "_model_config"]
...
class ResultStore(object):
    ...
    def remote_path_to_mesh(self, mesh_name, xid, split):
        return os.path.join(
            self.remote_result_ckpt_dir(xid),
            str(self.ckpt_for_xid(xid, split)),
            split,
            self._mesh_relative_path(mesh_name),
        )

    def local_path_to_mesh(self, mesh_name, xid, split):
        return os.path.join(
            self.local_result_ckpt_dir(xid),
            str(self.ckpt_for_xid(xid, split)),
            split,
            self._mesh_relative_path(mesh_name),
        )
    ...
    def local_qual_path(self):
        ...
        path = os.path.join(self.local_root, self.experiment.experiment_name, "images/")
