...
def get_npz_paths(split, category, modifier=""):
    ...
    logging_util.log.info("Loaded filelist in %0.2fms.", (time.time() - t))
    return filenames
...
def get_rgb_paths(split, category, modifier=""):
    ...
    logging_util.log.info("Time loading filelist %0.2f", (time.time() - t))
    return files
...
def rgb_path_to_npz_path(rgb_path, split, dataset="shapenet-occnet"):
    ...
    npz_path = "ROOT_DIR/%s/%s/%s/%s.npz" % (dataset, split, synset, mesh_hash)
    return npz_path
...
def read_png_to_float_npy_with_reraising(path):
    try:
        ...
        logging_util.log.info("Failed to load file %s", path)
