...
def select_image(images, indices):
    ...
    selected = jax.vmap(lambda x, y: x[y], in_axes=(0, 0))(images, indices)
...
class BoundingBox(object):
    ...
    def from_samples(cls, samples):
        ...
        """
        lower_bound = jnp.min(samples, axis=1, keepdims=True)
        upper_bound = jnp.max(samples, axis=1, keepdims=True)
...
class ShapeNetExample(object):
    ...
    def full_near_surface_samples(self):
        if not hasattr(self, "_full_near_surface_samples"):
            assert self._dataset.near_surface_samples.shape == (self._model_config.batch_size, 100000, 4)
            self._full_near_surface_samples = self._dataset.near_surface_samples

        return self._finite_wrapper(self._full_near_surface_samples)

    @property
    def full_uniform_samples(self):
        if not hasattr(self, "_full_uniform_samples"):
            assert self._dataset.bounding_box_samples.shape == (self._model_config.batch_size, 100000, 4)

            self._full_uniform_samples = self._dataset.bounding_box_samples
        return self._finite_wrapper(self._full_uniform_samples)

    def _subsample(self, samples, sample_count):
        """Returns a uniform random subsampling of an input sample set."""
        max_sample = samples.shape[1]
        ...
        subsamples = jax.vmap(lambda x, y: x[y], in_axes=(0, 0))(samples, sample_indices)
    ...
    def depth_render(self):
        ...
        if not hasattr(self, "_depth_render"):
            depth_render = ensure_shape_and_resize_if_needed(
                jnp.expand_dims(self._dataset.depth_render, axis=1),
            ...
            depth_render = depth_render.astype(jnp.float32)
    ...
    def apply_transformation(self, tx):
        ...
        (
            all_surface_points,
            all_surface_normals,
        ) = geom_util.transform_points_with_normals(all_surface_points, tx=tx, normals=all_surface_normals)
        ...
        if self._model_config.train or self._model_config.eval:
            ...
            local2grid = jnp.matmul(self.world2grid, local2world)
    ...
    def xyz_render(self):
        ...
        if not hasattr(self, "_xyz_render"):
            xyz_render = ensure_shape_and_resize_if_needed(
                jnp.expand_dims(self._dataset.xyz_render, axis=1),
                self._model_config.batch_size,
                1,
                3,
                self._model_config.gap_height,
            ...
            xyz_render = xyz_render.astype(jnp.float32)
    ...
    def depth_renders(self):
        ...
        if self._depth_renders is None:
            ...
            depth_renders = depth_renders.astype(jnp.float32)
    ...
    def random_depth_indices(self):
        if self._random_depth_indices is None:
            self._random_depth_indices = jax.random.uniform(
                self.next_key,
                shape=[
                    self._model_config.batch_size,
    ...
    def random_depth_to_worlds(self):
        ...
        if self._random_depth_to_worlds is None:
            self._random_depth_to_worlds = self.depth_to_world[
                jnp.reshape(
                    self.random_depth_indices,
                    [self._model_config.batch_size * self._model_config.num_random_images],
                ),
            ]
            self._random_depth_to_worlds = jnp.reshape(
    ...
    def depth_to_world(self):
        if self._depth_to_world is None:
            self._depth_to_world = geom_util.get_dodeca_camera_to_worlds().astype(jnp.float32)
            assert self._depth_to_world.shape == (20, 4, 4)
    ...
    def lum_renders(self):
        ...
        if self._lum_renders is None:
            self._lum_renders = ensure_shape_and_resize_if_needed(
    ...
    def random_lum_render(self):
        ...
        if self._random_lum_render is None:
            ...
            self._random_lum_render = jnp.reshape(
    ...
    def random_xyz_render(self):
        ...
        if not hasattr(self, "_random_xyz_render"):
            ...
            self._random_xyz_render = jnp.reshape(
    ...
    def sample_bounding_box(self):
        ...
        if self._bounding_box is None:
            ...
            bbox_samples = jnp.where(is_inside, bbox_samples, 0.0)
    ...
    def sample_sdf_near_surface(self, sample_count):
        ...
        return self._finite_wrapper(jnp.split(subsamples, [3], axis=2))
    ...
    def all_uniform_samples(self):
        return self._finite_wrapper(jnp.split(self.full_uniform_samples, [3], axis=2))

    def _set_points_and_normals(self):
        points, normals = jnp.split(self._full_zero_set_points_and_normals, [3], axis=2)
    ...
    def sample_zero_set_points_and_normals(self, sample_count):
        ...
        points, normals = jnp.split(subsamples, [3], axis=2)
