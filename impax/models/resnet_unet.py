...
def fixed_padding(inputs, kernel_size, data_format):
    ...
    if data_format == "channels_first":
        padded_inputs = jnp.pad(inputs, [[0, 0], [0, 0], [pad_beg, pad_end], [pad_beg, pad_end]])
    else:
        padded_inputs = jnp.pad(inputs, [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]])
...
class ResnetUnet(nn.Module):
    ...
    def __call__(self, x):
        ...
        if self.flat_predict:
            ...
            prediction = jnp.reshape(prediction, [batch_size, self.sc, self.param_count])
            return prediction
        ...
        if data_format == "channels_first":
            flat_out_4x4 = jnp.reshape(
                jnp.transpose(output_4x4, perm=[0, 2, 3, 1]), [batch_size, 4 * 4, self.param_count]
            )
            flat_out_7x7 = jnp.reshape(
                jnp.transpose(output_7x7, perm=[0, 2, 3, 1]), [batch_size, 7 * 7, self.param_count]
            )
            flat_out_14x14 = jnp.reshape(
                jnp.transpose(output_14x14, perm=[0, 2, 3, 1]), [batch_size, 14 * 14, self.param_count]
            )
            flat_out_28x28 = jnp.reshape(
                jnp.transpose(output_28x28, perm=[0, 2, 3, 1]), [batch_size, 28 * 28, self.param_count]
            )
            flat_out_56x56 = jnp.reshape(
                jnp.transpose(output_56x56, perm=[0, 2, 3, 1]), [batch_size, 56 * 56, self.param_count]
            )
        else:
            flat_out_4x4 = jnp.reshape(output_4x4, [batch_size, 4 * 4, self.param_count])
            flat_out_7x7 = jnp.reshape(output_7x7, [batch_size, 7 * 7, self.param_count])
            flat_out_14x14 = jnp.reshape(output_14x14, [batch_size, 14 * 14, self.param_count])
            flat_out_28x28 = jnp.reshape(output_28x28, [batch_size, 28 * 28, self.param_count])
