...
def test_marching_cubes(resolution=4, mcubes_extent=1.0, key=random.PRNGKey(0)):
    volume = random.uniform(key, shape=(resolution, resolution, resolution))
    ground_truth0, ground_truth1 = original.marching_cubes(
        tf.convert_to_tensor(volume), mcubes_extent
    )
