...
def test_point_iou():
    ...
    quadric_count = model_config.hparams.sc
    ...
    tf_structure_imp = orig_sdf.StructuredImplicit(model_config, constants, centers, radii, iparams, net)
    gt = orig.point_iou(tf_structure_imp, sample_locations, sample_locations, model_config)

    structured_implicit = structured_implicit_functions.StructuredImplicit(
        constants, centers, radii, iparams, net, model_config
    )
    ret = metrics.point_iou(structured_implicit, sample_locations, sample_locations, model_config)
