...
def _unflatten(vector):
    ...
    radius_len = int(jnp.prod(radius_shape))
...
def element_explicit_dof(model_config):
    ...
    if model_config.hparams.r == "iso":
        ...
        raise ValueError(
            "Unrecognized radius hyperparameter: %s" % model_config.hparams.r
        )
...
def ensure_jnp_shape(arr, expected_shape, name):
...
class StructuredImplicitjnp(object):
    ...
    def flat_vector(self):
        ...
        return jnp.concatenate(l, axis=-1)
        ...
        for li in range(block_count):
            ...
            for hi in range(block_count):
                ...
                for wi in range(block_count):
                    ...
                    offset = jnp.reshape(
                        jnp.array([w_min, l_min, h_min], dtype=jnp.float32), [1, 1, 1, 3]
                    )
    ...
    def _build_sample_on_grid(self):
        ...
        self._predicted_class_grid = tf.reshape(
...
def _tile_for_symgroups( elements):
...
def _generate_symgroup_samples( samples):
    ...
    samples = jax_util.tile_new_axis(samples, axis=1, length=model_config.hparams.sc)
...
class StructuredImplicit(object):
    ...
    def __init__(self,  constants, centers, radii, iparams, net):
    ...
    @classmethod
    def from_activation(cls,  activation, net):
        ...
        if model_config.hparams.r == "cov":
            ...
            radius_rot = tf.clip_by_value(
                radius[..., 3:], -max_euler_angle, max_euler_angle
            )
    ...
    def force_valid_values(self):
        ...
        if self._model_config.hparams.r == "cov":
            ...
            rotr = tf.clip_by_value(rotr, -jnp.pi / 4.0, jnp.pi / 4.0)
    ...
    def flat_vector(self):
        ...
        if self._flat_vector is None:
            ...
            self._flat_vector = tf.reshape(
                self.vector, [self._model_config.hparams.bs, sc, sd]
            )
    ...
    def zero_constants_by_threshold(self, threshold):
        ...
        Returns:
          No return value.
        """
        self._constants = tf.where(
            self._constants >= threshold,
            tf.zeros_like(self._constants),
            self._constants,
        )
    ...
    def zero_constants_by_volume(self, volume_threshold):
        ...
        self._constants = tf.where(
            should_zero, tf.zeros_like(self._constants), self._constants
        )
    ...
    def implicit_values(self, local_samples):
        ...
        if self._model_config.hparams.ipe == "f":
            ...
            raise ValueError(
                "Unrecognized ipe hparam: %s" % self._model_config.hparams.ipe
            )
        else:
            iparams = _tile_for_symgroups(self._ self._iparams)
            eec = iparams.shape[-2]
    ...
    def rbf_influence_at_samples(self, samples):
        ...
        effective_constants = _tile_for_symgroups(self._ self._constants)
        effective_centers = _tile_for_symgroups(self._ self._centers)
        effective_radii = _tile_for_symgroups(self._ self._radii)
        # Gives the samples shape [batch_size, effective_elt_count, sample_count, 3]
        effective_samples = _generate_symgroup_samples(self._ samples)
