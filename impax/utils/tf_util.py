...
# not necessary, we don't use tensors and we use python's assert 
# tensor : jnp array
def assert_shape(tensor, shape, name):
  real_shape = jnp.shape(tensor).as_list()
  ...
  if not same_rank or not all_equal:
    raise ValueError('Error: Expected tensor %s to have shape %s, but it had shape %s.' %
...
def zero_by_mask(mask, vals, replace_with=0.0):
  ...
  """
  assert mask.dtype == jnp.dtype(jnp.bool)
  ms = jnp.shape(mask).as_list()
  vs = jnp.shape(vals).as_list()
  assert ms == vs[::-1] + [1]
  assert vs == ms[::-1] + [vs[-1]]
#  mask = tf.ensure_shape(mask, vs[:-1] + [1])
#  vals = tf.ensure_shape(vals, ms[:-1] + [vs[-1]])
  vals = jnp.where(mask, vals, replace_with)
  return vals
...
def remove_element(t, elt, axis):
  ...
  elt_tensor = jnp.reshape(elt_tensor, [1] * num_dims_before_axis +
