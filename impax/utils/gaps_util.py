...
References:
https://github.com/google/ldif/blob/master/ldif/util/gaps_util.py
"""
...
def read_cam_file(path, verbose=False):
    ...
    for i, l in enumerate(lines):
        ...
        viewpoint = jnp.array(vals[0:3]).astype(jnp.float32)
        towards = jnp.array(vals[3:6]).astype(jnp.float32)
        up = jnp.array(vals[6:9]).astype(jnp.float32)
    ...
    xfovs = jnp.array(xfovs, dtype=jnp.float32)
...
def batch_gaps_depth_image_to_cam_image(depth_image, xfov=None):
    ...
    if isinstance(xfov, float):
        xfov = jnp.ones([batch_size], dtype=jnp.float32) * xfov
    else:
        assert len(xfov.shape) in [0, 1]
        if not xfov.shape:
            xfov = jnp.reshape(xfov, [1])
        if xfov.shape[0] == 1:
            xfov = jnp.tile(xfov, [batch_size])
        assert xfov.shape[0] == batch_size
    out = []
    for i in range(batch_size):
        out.append(gaps_depth_image_to_cam_image(depth_image[i, ...], xfov[i]))
...
def gaps_depth_image_to_cam_image(depth_image, xfov=0.5):
    ...
    cam_z = nic_d
    return jnp.stack([cam_x, cam_y, cam_z], axis=2)
...
def read_normals_im(px, py, pz):
    ...
    ns = jnp.stack(ns, axis=-1)
    norm = jnp.linalg.norm(ns, axis=-1, keepdims=True)
    ns /= jnp.maximum(norm, 1e-8)
    return ns
...
def read_normals_dir(directory, image_count):
    ...
    for i in range(image_count):
        ...
        image = read_normals_im(f"{base}_nx.png", f"{base}_ny.png", f"{base}_nz.png")
        images.append(image)
...
def transform_r2n2_depth_image_to_gaps_frame(depth_im, idx, e):
    ...
    # Cam2world
    xyz_im = geom_util_jnp.apply_4x4(
        cam_im, e.r2n2_cam2world[idx, ...], are_points=True
    )
    mask = jnp_util.make_mask(depth_im)
    xyz_im = jnp_util.zero_by_mask(mask, xyz_im).astype(jnp.float32)
    return xyz_im
