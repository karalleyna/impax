...
def interpolate_np(grid, samples, world2grid):
    ...
    """
    xyzw_samples = jnp.pad(samples, [[0, 0], [0, 1]], mode="constant", constant_values=1)
    ...
    c00 = grid[lca[0], lca[1], lca[2]] * (1 - aca[0]) + grid[uca[0], lca[1], lca[2]] * aca[0]
    c01 = grid[lca[0], lca[1], uca[2]] * (1 - aca[0]) + grid[uca[0], lca[1], uca[2]] * aca[0]
    c10 = grid[lca[0], uca[1], lca[2]] * (1 - aca[0]) + grid[uca[0], uca[1], lca[2]] * aca[0]
    c11 = grid[lca[0], uca[1], uca[2]] * (1 - aca[0]) + grid[uca[0], uca[1], uca[2]] * aca[0]
...
def interpolate(grid, samples, world2grid):
    return vmap(interpolate_np, in_axes=(0, 0, 0))(grid, samples, world2grid)
