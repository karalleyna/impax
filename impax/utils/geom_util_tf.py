...
def local_views_of_shape(global_points,
  ...
  if zeros_invalid:
    ...
    global_points = tf.where(is_zero, 100.0, global_points)
  ...
  if not expand_region:
    local_points = tf.where(points_valid, local_points, 0.0)
...
def transform_depth_dodeca_to_xyz_dodeca(depth_dodeca):
  ...
  xyz_images = tf.where(depth_dodeca > 0.0, xyz_images, 0.0)
  return xyz_images
...
def depth_image_to_xyz_image(depth_images, world_to_camera, xfov=0.5):
  ...
  camera_to_world_mat = tf.linalg.inv(world_to_camera)
...
def interpolate_from_grid_coordinates(samples, grid):
  ...
  alphas = tf.math.floormod(offset_samples, 1.0)
  ...
  validity_mask = tf.reduce_min(
      tf.cast(is_valid, dtype=tf.float32), axis=2)
  ...
  for xi, x_coord in enumerate([lower_coords[:, :, 0], upper_coords[:, :, 0]]):
    ...
    for yi, y_coord in enumerate([lower_coords[:, :, 1], upper_coords[:, :,
      ...
      for zi, z_coord in enumerate(
        ...
        lookup_result = tf.gather(flattened_grid, flat_lookup,batch_dims=tf.rank(flat_lookup) - 1)
